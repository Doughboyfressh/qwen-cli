...
class ContextSnapshot:
    ...
    @staticmethod
    def create(history: list, classifier: ImportanceClassifier, 
    ...
    @staticmethod
    def load_recent(session_id: Optional[str] = None, max_count: int = 3) -> list:
...
def importance_aware_truncate(history: list, 
                               target_reduction: float = 0.3,
                               keep_first: int = 6,
                               keep_last: int = 20) -> tuple[list, dict]:
    ...
    chat_msgs = [m for m in tagged if m.get("role") not in ("system",)]
